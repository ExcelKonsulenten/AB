...
def robust_read_csv_from_bytes(name: str, data: bytes) -> pd.DataFrame:
    """Prøver ulike separatorer og encodings fra bytes-buffer."""
    seps, encs = [";", ","], ["utf-8-sig","latin-1","cp1252"]
    last_err = None
    for sep in seps:
        for enc in encs:
            try:
                bio = BytesIO(data)  # viktig: reset for hvert forsøk
                return pd.read_csv(bio, sep=sep, encoding=enc, low_memory=False)
            except Exception as e:
                last_err = e
                continue
    st.error(f"Kunne ikke lese {name}. Siste feil: {last_err}")
    return pd.DataFrame()

def normalize_columns(df): 
    return df.rename(columns={c: COL_MAP.get(c.strip().lower(), c.strip()) for c in df.columns})
...
def load_data():
    """NY: Les data fra file_uploader i stedet for fra mappen."""
    st.sidebar.header("Data")
    uploads = st.sidebar.file_uploader(
        "Last opp CSV-fil(er) (f.eks. eksport fra systemet)",
        type=["csv"],
        accept_multiple_files=True,
        help="Du kan laste opp én eller flere filer. De blir slått sammen automatisk."
    )
    if not uploads:
        st.info("👆 Last opp én eller flere CSV-filer i sidepanelet for å starte.")
        return pd.DataFrame()
    ...
    for uf in uploads:
        data = uf.read()
        if not data:
            st.warning(f"{uf.name}: tom fil.")
            continue
        df = robust_read_csv_from_bytes(uf.name, data)
        if df.empty: 
            continue
        df = normalize_columns(df)
        df["__kildefil"] = uf.name
        parts.append(df)
    ...
    df = coerce_dates(df)
    df["Avdeling"] = df.apply(derive_avdeling, axis=1)

    res = df.get("Totalt betalt", pd.Series([], dtype=float)).apply(payments_from_total)
    if len(res):
        df["Antall betalinger"] = [a for a,_ in res]
        df["Avvik"] = [b for _,b in res]
    else:
        df["Antall betalinger"]=np.nan
        df["Avvik"]=True
...
def apply_filters(df: pd.DataFrame):
    ...
    if "Startdato" in df.columns:
        ...
        if year != "Alle":
            df = df[pd.to_datetime(df["Startdato"]).dt.year == int(year)]
    ...
    # Antall betalinger
    if "Antall betalinger" in df.columns and df["Antall betalinger"].notna().any():
...
def compute_survival_and_dropouts(df):
    ...
    for n in range(1,max_n+1): 
        xs.append(n); ys.append(int((agg["Antall_betalinger"].fillna(0)>=n).sum()))
...
with st.expander("Se rådata (filtrerbar)",expanded=False): 
    st.dataframe(df)
...
with colB:
    ...
    if "Betalingsmåte" not in fdf.columns:
        ...
        if agg.empty:
            ...
            fig2 = plt.figure()
            ax2 = plt.gca()
            for meth, base_m in bases.items():
                ...
                if vis_prosent and base_m>0:
                    ys = [y / base_m * 100.0 for y in ys]
